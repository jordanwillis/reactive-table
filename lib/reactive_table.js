...
set = function(object, field, value) {
  var keys = field.split('.');
  var last = keys.pop();

  var obj = _.reduce(keys, function(a, b) {
    if (a[b] == null) {
      a[b] = {};
    }
...
var setup = function () {
    ...
    context.showExportData = getDefaultFalseSetting('showExportData', this.data);
    context.exportDataTitle = this.data.exportDataTitle || this.data.settings.exportDataTitle || 'Export';
...
var exportData = function () {
  ...
  if (sortByValue) {
      ...
      jsonData = this.collection.find(filterQuery, {
          sort: sortQuery,
      }).fetch();
  } else {
      var rows = this.collection.find(filterQuery).fetch();
      jsonData = sortWithFunctions(rows, this.fields, this.multiColumnSort);
  }
  ...
  _.each(template.context.visibleFields.get(), function(e) {
    exportFields.push(_.findWhere(self.fields, {fieldId: e.fieldId}));
  });

  var exportFnFields = _.filter(exportFields, function(e) {
    return e.exportFn != null;
  });

  var tmpData, tmpField, fieldConfig;

  if (exportFnFields && exportFnFields.length > 0) {
    for (var i = 0; i < jsonData.length; i++) {
      tmpData = {};

      for (var j = 0; j < exportFnFields.length; j++) {
        fieldConfig = exportFnFields[j];
        set(tmpData, fieldConfig.key, fieldConfig.exportFn(jsonData[i]));
      }

      for (j = 0; j < exportFnFields.length; j++) {
        fieldConfig = exportFnFields[j];
        tmpField = get(tmpData, fieldConfig.key);
        set(jsonData[i], fieldConfig.key, tmpField);
      }
    }
  }

  jsonToCsvExport(jsonData, exportFields, template.context.exportDataTitle);
};
...
Template.reactiveTable.events({
    ...
    'click .reactive-table-navigation .export-data': function (event) {
        var template = Template.instance();
